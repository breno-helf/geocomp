...
def make_events(polygons, evts, root):
    lp = []
    rp = []
...
def make_slabs(s, events, root):
    ...
    abb_Root = None
    curr = Slab()
    cel = Hor_line([DMIN, DMAX, DMAX, DMAX, -1])
    abb.insert(abb_Root, cel, 0)
    curr.end = DMIN
    v = []
    events.inOrder(root, v)
...
def main():
    ...
    make_events(polygons, events, eventsRoot)
